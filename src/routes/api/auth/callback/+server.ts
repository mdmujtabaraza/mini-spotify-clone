...
export const GET: RequestHandler = async ({ url, cookies, fetch }) => {
  ...
  const response = await fetch("https://accounts.spotify.com/api/token", {
    ...
    headers: {
      ...
      Authorization:
        "Basic " +
        btoa(`${SPOTIFY_APP_CLIENT_ID}:${SPOTIFY_APP_CLIENT_SECRET}`),
    },
  ...
  // cookies.set('refresh_token', responseJSON.refresh_token, { path: '/', httpOnly: true, sameSite: 'strict', secure: true })
